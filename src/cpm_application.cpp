      ...
      for (autoware_auto_perception_msgs::msg::PredictedObject obj : msg->objects) {
        ...
        if (existence_probability >= objectConfidenceThreshold_) {
          ...
          if (found_object == objectsList.end()) {
            ...
            object.position_x = obj.kinematics.initial_pose.pose.position.x;
            object.position_y = obj.kinematics.initial_pose.pose.position.y;
            object.position_z = obj.kinematics.initial_pose.pose.position.z;
            object.orientation_x = obj.kinematics.initial_pose.pose.orientation.x;
            object.orientation_y = obj.kinematics.initial_pose.pose.orientation.y;
            object.orientation_z = obj.kinematics.initial_pose.pose.orientation.z;
            object.orientation_w = obj.kinematics.initial_pose.pose.orientation.w;
            object.shape_x = std::lround(obj.shape[0].polygon.points[0].x * 10.0);
            object.shape_y = std::lround(obj.shape[0].polygon.points[0].y * 10.0);
            object.shape_z = std::lround(obj.shape[0].polygon.points[0].z * 10.0);
...
//            object.shape_z = 0;//std::lround(obj.shape.dimensions.z * 10.0);
            ...
            if (0 == autoware_auto_perception_msgs::msg::ObjectClassification::PEDESTRIAN || 0 == autoware_auto_perception_msgs::msg::ObjectClassification::UNKNOWN) {
              ...
              double dist = pow(obj.kinematics.initial_pose.pose.position.x - found_object->position_x, 2) + pow(obj.kinematics.initial_pose.pose.position.y - found_object->position_y, 2);
              dist = sqrt(dist);
              ...
              double speed = pow(obj.kinematics.initial_twist.twist.linear.x - found_object->twist_linear_x, 2) + pow(obj.kinematics.initial_twist.twist.linear.x- found_object->twist_linear_y, 2);
              speed = sqrt(speed);
              ...
              double twist_angular_x_diff = (obj.kinematics.initial_twist.twist.angular.x - found_object->twist_angular_x) * 180 / M_PI;
            ...
            found_object->position_x = obj.kinematics.initial_pose.pose.position.x;
            found_object->position_y = obj.kinematics.initial_pose.pose.position.y;
            found_object->position_z = obj.kinematics.initial_pose.pose.position.z;
            found_object->orientation_x = obj.kinematics.initial_pose.pose.orientation.x;
            found_object->orientation_y = obj.kinematics.initial_pose.pose.orientation.y;
            found_object->orientation_z = obj.kinematics.initial_pose.pose.orientation.z;
            found_object->orientation_w = obj.kinematics.initial_pose.pose.orientation.w;
...
//            found_object->shape_z = std::lround(obj.shape.dimensions.z * 10.0);
            found_object->shape_x = std::lround(obj.shape[0].polygon.points[0].x * 10.0);
            found_object->shape_y = std::lround(obj.shape[0].polygon.points[0].y * 10.0);
            found_object->shape_z = std::lround(obj.shape[0].polygon.points[0].z * 10.0);
